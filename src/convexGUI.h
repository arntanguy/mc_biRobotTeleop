...
// TODO: upstream to mc_rtc
namespace birobot_teleop::gui
{

inline void addConvexToGUI(mc_rtc::gui::StateBuilder & gui,
                           const std::vector<std::string> & category,
                           const mc_rbdyn::Robot & robot,
                           const std::shared_ptr<sch::S_Object> & convex,
                           const std::string & name,
                           const std::string & body,
                           const mc_rtc::gui::PolyhedronConfig & cfg,
                           const std::optional<std::string> & publishName = std::nullopt,
                           const sva::PTransformd & collisionTransform = sva::PTransformd::Identity())
{
  const auto & c = convex;
  auto get_pose = [&, name, body, collisionTransform]() { return collisionTransform * robot.bodyPosW(body); };
  auto publish_object = [&](auto && element) { gui.addElement(category, element); };
  // NOTE:
  // we only access values for the S_Object, however since getPolyhedronAlgorithm is not marked const,
  // we need to explicitely cast away constness
  if(auto poly = std::dynamic_pointer_cast<sch::S_Polyhedron>(const_cast<std::shared_ptr<sch::S_Object> &>(c)))
  {
...
inline void addConvexToGUI(mc_rtc::gui::StateBuilder & gui,
                           ...
                           const std::optional<std::string> & publishName = std::nullopt)
{
  addConvexToGUI(gui, category, robot, robot.convex(name).second, name, robot.convex(name).first, cfg, publishName);
}
